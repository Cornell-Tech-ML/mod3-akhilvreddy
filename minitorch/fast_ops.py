        ...
        b_strides: Strides,
            ...
            if (len(out_shape) == len(a_shape) == len(b_shape) and 
                ...
                for i in prange(int(np.prod(out_shape))):
                    ...
                    if (all(0 <= a_index[d] < a_shape[d] for d in range(len(a_shape))) and
                        all(0 <= b_index[d] < b_shape[d] for d in range(len(b_shape)))):
...
def _tensor_matrix_multiply(
    ...
    * Inner loop should have no global writes, 1 multiply.
    ...
    n_batches = max(a_shape[0] if len(a_shape) > 2 else 1, b_shape[0] if len(b_shape) > 2 else 1)
    row_size = a_shape[-2]
